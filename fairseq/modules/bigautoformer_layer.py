...
class BigAutoformerEncoderLayer(nn.Module):
    ...
    def forward(self, curr_x, prev_x, post_x,
        ...
        """
        ctx_x = torch.cat((prev_x, post_x), dim=0)
        ctx_encoder_padding_mask = torch.cat((prev_encoder_padding_mask, post_encoder_padding_mask), dim=1)
        ctx_x = self.context_encoder_layer(ctx_x, ctx_encoder_padding_mask)

        length = prev_x.size(0)
