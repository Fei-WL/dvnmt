...
class FairseqDVNMTEncoderDecoderModel(BaseFairseqModel):
    """Base class for encoder-decoder models.

    Args:
        encoder (FairseqEncoder): the encoder
        tgtencoder (FairseqEncoder): the encoder of tgt without train at the first time
        decoder (FairseqDecoder): the decoder
    """

    def __init__(self, ctx_encoder, src_encoder, tgt_encoder, src_discriminator, 
        tgt_discriminator, prior_net, posterior_net, decoder):
        super().__init__()
        ...
        self.tgt_discriminator = tgt_discriminator
        self.prior_net = prior_net
        self.posterior_net = posterior_net
        self.decoder = decoder

    def forward(self, src_tokens, src_lengths, prev_tokens, prev_output_tokens, **kwargs):
        """
        Run the forward pass for an encoder-decoder model.
        ...
        Args:
            src_tokens (LongTensor): tokens in the source language of shape
                `(batch, src_len)`
            src_lengths (LongTensor): source sentence lengths of shape `(batch)`
            prev_tokens (LongTensor): tokens in the source language of shape
                `(batch, src_len)`
            prev_output_tokens (LongTensor): previous decoder outputs of shape
                `(batch, tgt_len)`, for teacher forcing
